...
public class CacheConfig extends CachingConfigurerSupport {
	...
	@Bean("complianceBarCache")
	public Cache cache(CacheManager cacheManager) {
		return cacheManager.getCache("compliance-bar");
	}

	@Bean("complianceBarRateLimitCache")
	public Cache cacheRateLimit(CacheManager cacheManager) {
		return cacheManager.getCache("compliance-bar-rate-limit");
	}
	...
	public ClientConfig clientConfigTest() {
		...
		clientConfig.getNetworkConfig()
				.addAddress("compliance-bar-hazelcast:5701")
				.setSmartRouting(true);
		return clientConfig;
	}
